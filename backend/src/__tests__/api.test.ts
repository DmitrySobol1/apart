...
async function buildApp(): Promise<express.Express> {
  ...
  const [roomsRouter, plansRouter, amenitiesRouter, accountRouter, bookingRouter, { errorHandler }] =
    await Promise.all([
...
describe('Backend API', () => {
  ...
  describe('POST /api/booking validation', () => {
    ...
    it.each([
      ...
      ['invalid phone format', { ...validBooking, guest: { ...validBooking.guest, phone: '89001234567' } }],
      ['invalid email', { ...validBooking, guest: { ...validBooking.guest, email: 'not-an-email' } }],
