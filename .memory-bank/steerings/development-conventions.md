...
- **Two packages**: `frontend/` (React + Vite) and `backend/` (Express)
- **No circular dependencies** between frontend and backend
- **Shared types**: Duplicate minimal shared types in both packages (no shared package in MVP)
- **Minimize dependencies**: Add a lib only if it saves significant effort and adds no heavy transitive risks
...
- Axios (HTTP client to Bnovo API)
...
- **Quality check commands**: Defined in `.memory-bank/steerings/project-commands.md`
